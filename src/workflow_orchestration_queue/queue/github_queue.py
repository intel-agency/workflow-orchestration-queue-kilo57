...
See:
    ...
    - OS-APOW Simplification Report, S-1 / S-6
"""
...
class ITaskQueue(ABC):
    ...
    This abstraction allows the queue provider to be swapped without
    modifying the Sentinel Orchestrator or Work Event Notifier logic.

    See ADR 09 for the full rationale on task queue abstraction.
    """
    ...
    async def fetch_queued_tasks(self) -> list[WorkItem]:
        ...
        Returns:
            A list of work items currently in the queued state.
...
class GitHubQueue(ITaskQueue):
    ...
    Attributes:
        ...
        org: GitHub organization name (required for polling).
        repo: GitHub repository name (required for polling).
    ...
    def __init__(self, token: str, org: str = "", repo: str = "") -> None:
        ...
        Args:
            ...
            org: GitHub organization name (optional for webhook-based usage).
            repo: GitHub repository name (optional for webhook-based usage).
    ...
    async def add_to_queue(self, item: WorkItem) -> bool:
        ...
        """
        base_url = self._repo_api_url(item.target_repo_slug)
        url = f"{base_url}/issues/{item.issue_number}/labels"
    ...
    async def claim_task(
        ...
        This implements optimistic concurrency control for distributed
        sentinel instances competing for the same work items.
        ...
        Args:
            ...
            bot_login: GitHub username of the bot to assign (optional).
    ...
    async def post_heartbeat(
        self, item: WorkItem, sentinel_id: str, elapsed_secs: int
    ) -> None:
        ...
        Args:
            ...
            sentinel_id: Unique identifier for the sentinel instance.
            elapsed_secs: Number of seconds elapsed since task start.
